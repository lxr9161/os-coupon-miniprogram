	...
	<view>
		<view class="nl_top-ads">
			<swiper v-if="ads.length > 0" class="swiper" :indicator-dots="true" :autoplay="true" :interval="3000" :duration="500" :style="'height:' + swiperHeight">
				<swiper-item v-for="(item, key) in ads" :key="key">
					...
					<navigator v-else-if="item.type === 1 && item.target === 1" open-type="navigate" target="miniProgram" :app-id="item.appid" :path="item.link">
						<view>
							<image class="nl_ad-image" :src="imgHost + item.img_url" mode="widthFix" @load="loadImage"></image>
						</view>
					</navigator>
					<navigator v-else-if="item.type === 1 && item.target === 0" open-type="navigate" :url="item.link">
						<view>
							<image class="nl_ad-image" :src="imgHost + item.img_url" mode="widthFix" @load="loadImage"></image>
						</view>
					</navigator>
		...
		<view class="nl_section" v-if="systemSetting.index_coupon_display === 1">
			<view class="nl_section-head">
				...
				<view class="nl_section-title">领红包</view>
				<view class="nl_remind-switch-box">
					<view class="nl_remind-switch-text" v-if="!openAllNocticeStatus">优惠提醒</view>
					<image src="../../static/remind_2.png" class="nl_remind-switch-icon"></image>
					<switch class="nl_remind-switch" color="#fe5656" @change="openAllNotice" :checked="openAllNocticeStatus"></switch>
				</view>
			</view>
		...
		<view class="nl_section">
			<view class="nl_section-head nl_mb-20">
				...
				<view class="nl_remind-switch-box">
					<view class="nl_remind-switch-text" v-if="!openAllNocticeStatus">开启提醒</view>
					<image src="../../static/remind_2.png" class="nl_remind-switch-icon"></image>
					<switch class="nl_remind-switch" color="#fe5656" @change="openAllNotice" :checked="openAllNocticeStatus"></switch>
				</view>
			</view>
	...
	export default {
		...
		data() {
			return {
				...
				swiperHeight: '',
				ads: [],
				tmpIds: getApp().globalData.templateIds
			}
		},
		methods: {
			// 获取广告图片
			...
			openAllNotice (event) {
				let that = this
				this.$set(this, 'openAllNocticeStatus', event.target.value)
				if (event.target.value) {
					uni.requestSubscribeMessage({
						tmplIds: this.tmpIds,
						success: (res) => {
							let status = true
							this.tmpIds.map((item) => {
								if (res[item] === 'reject') {
									status = false
								}
							})
							this.$set(this, 'openAllNocticeStatus', status)
							this.setClockNotice()
						}
					})
				}

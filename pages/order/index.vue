	...
	<view>
		<view class="nl_order-list">
			<view class="nl_order-item" v-for="(item, key) in orderList" :key="key">
				...
				<view class="nl_order-pirce">
					订单金额:<text class="nl_bold nl_ml-6">{{ item.pay_price }}</text>
					<text class="nl_order-type">{{ item.order_type }}</text>
				</view>
				<view class="nl_order-profit">
					<view>
						<text class="nl_bold-text">返利:</text><text class="nl_bold-red nl_ml-6">{{ item.profit }}</text>
					</view>
